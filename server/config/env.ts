...
const envSchema = z.object({
  ...
  // ── LLM Gateway ──────────────────────────────────────────────────────────
  LLM_DEFAULT_TIMEOUT_MS: z.string().transform(Number).default("30000"),
  LLM_STREAM_TIMEOUT_MS: z.string().transform(Number).default("180000"),
  LLM_STREAM_IDLE_TIMEOUT_MS: z.string().transform(Number).default("30000"),
  LLM_CACHE_TTL_MS: z.string().transform(Number).default("120000"),
  LLM_MAX_CONTEXT_TOKENS: z.string().transform(Number).default("12000"),
  LLM_IN_FLIGHT_TIMEOUT_MS: z.string().transform(Number).default("60000"),
  LLM_RETRY_MAX_RETRIES: z.string().transform(Number).default("2"),
  LLM_RETRY_BASE_DELAY_MS: z.string().transform(Number).default("200"),
  LLM_RETRY_MAX_DELAY_MS: z.string().transform(Number).default("3000"),
  LLM_RETRY_JITTER_FACTOR: z.string().transform(Number).default("0.15"),
  ...
  LLM_PROVIDER_MAX_CONCURRENCY: z.string().transform(Number).default("16"),
  LLM_PROVIDER_MAX_QUEUE: z.string().transform(Number).default("128"),
