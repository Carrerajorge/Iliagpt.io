...
async function resolveUserAccessProfile(userId?: string | null): Promise<UserAccessProfile> {
  if (process.env.NODE_ENV !== "production") {
    return { plan: "admin", isAdmin: true, isPaid: true };
  }
...
async function loadBaseCatalog(): Promise<BaseCatalogState> {
  ...
  let allModels: any[] = [];
  try {
    allModels = await storage.getAiModels();
  } catch (e) {
    // DB unavailable — fall back to curated presets only
    allModels = [];
  }
  ...
  let configuredDefaultModel: string;
  try {
    configuredDefaultModel = await getSettingValue<string>("default_model", FREE_MODEL_ID);
  } catch {
    configuredDefaultModel = FREE_MODEL_ID;
  }
  const defaultModel =
