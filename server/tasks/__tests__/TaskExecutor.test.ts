...
import { TaskExecutor, resolveTaskModel } from '../TaskExecutor';
import { getAgenticModelReadiness, initModelWiring, NO_LLM_PROVIDER_MESSAGE } from '../../integration/modelWiring';
...
describe('agentic model readiness', () => {
  const originalEnv = {
    DEFAULT_AGENT_MODEL: process.env.DEFAULT_AGENT_MODEL,
    XAI_API_KEY: process.env.XAI_API_KEY,
    OPENAI_API_KEY: process.env.OPENAI_API_KEY,
  ...
  it('reports a clear readiness error when no providers are configured', () => {
    const readiness = getAgenticModelReadiness();
    expect(readiness.ok).toBe(false);
    expect(readiness.reason).toBe(NO_LLM_PROVIDER_MESSAGE);
  });

  it('fails background task execution before starting the loop when providers are unavailable', async () => {
    const executor = new TaskExecutor();
    await expect(executor.execute(makeTask())).rejects.toThrow(NO_LLM_PROVIDER_MESSAGE);
  });
});
