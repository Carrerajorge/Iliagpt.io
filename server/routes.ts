...
import { generateGatewayToken } from "./services/openclawGateway";
...
rootObserver.observe(document.documentElement,{childList:true,subtree:true});
    ...
    async function serveControlUiWithAutoConnect(req: Request, res: Response) {
      try {
        ...
        // autoConnectScript removed: the OpenClaw 2026.4.14 bundle has
        // native auto-connect via iO(e).finally(YD). Injecting a DOM-level
        // click handler raced with the native connect and caused duplicate
        // WebSocket connections that cancelled each other mid-handshake.
    ...
    app.get("/openclaw-ui/assets/:file", (req: Request, res: Response, next: Function) => {
      ...
      // Patch 5: Single, guarded auto-connect.
      //
      // The Control UI's native mount path (iO(e).finally(YD)) is async
      // and in practice sometimes loses the race in embedded/iframe
      // contexts — the page mounts, the login gate renders, and the
      // native auto-connect never fires before the user gives up. We
      // inject one extra trigger that:
      //   - fires AT MOST ONCE per page lifetime (window.__OC_AUTOCONNECT_FIRED__)
      //   - only fires if `e.connect` is a function AND `e.settings.gatewayUrl`
      //     is present (pre-seed guaranteed this)
      //   - defers 600ms to let Lit finish its first mount pass
      //   - re-checks `!e.connected` inside the timeout so we skip if the
      //     native path already connected in the meantime.
      const p5 = js.replace(
        'if(CM=n,!e.connected)return i` ${iM(e)} ${tM(e)} `;',
        'if(CM=n,!e.connected){try{if(!window.__OC_AUTOCONNECT_FIRED__&&typeof e.connect===\'function\'&&e.settings&&e.settings.gatewayUrl){window.__OC_AUTOCONNECT_FIRED__=true;setTimeout(function(){try{if(!e.connected&&typeof e.connect===\'function\')e.connect()}catch(_e){}},600);}}catch(_e){}return i` ${iM(e)} ${tM(e)} `;}'
      );
