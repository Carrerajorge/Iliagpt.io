...
**Live OpenRouter API Integration** (`server/services/aiModelSyncService.ts`): Fetches the complete model catalog (342+ models across 57+ providers) live from `https://openrouter.ai/api/v1/models`. Functions: `fetchOpenRouterModels()` (raw API fetch with 30s timeout), `syncFromOpenRouter(options)` (sync to DB with provider filter, free-only filter, dry-run mode). Infers model type from architecture modality (TEXT/IMAGE/MULTIMODAL/AUDIO/EMBEDDING). Extracts provider from model ID slug. Public API: `GET /api/openrouter/models?search=&provider=&free=true&page=&limit=` (paginated catalog browser). Admin API: `GET /api/admin/models/openrouter/catalog` (full catalog), `POST /api/admin/models/openrouter/sync` (sync to DB with filters).
