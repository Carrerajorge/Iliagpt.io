...
**OpenClaw v2026.2.26 Agentic Tool Integration** — Core agentic tools from OpenClaw are integrated directly into `server/agents/` (not via API). The tool engine at `server/agents/toolEngine.ts` provides a central dispatcher with OpenAI function-calling format tools: `bash` (sandboxed shell execution), `web_fetch` (HTTP + HTML stripping), `web_search` (DuckDuckGo), `read_file`, `write_file`, `edit_file`, `list_files` (with path traversal protection). These tools are merged into the agent executor's tool array alongside the existing tool registry tools, and are dispatched via the `executeOpenClawToolCall` function. The `server/agent/agentExecutor.ts` now uses OpenAI-compatible API calls through OpenRouter (replacing the previous Gemini 2.0 Flash calls), routing through `OPENAI_BASE_URL` with `minimax/minimax-m2.5` as the default model (configurable via `AGENT_MODEL` env var).

**File Preview System** — Enhanced file preview in the composer bar uses `getFileTheme()` for consistent styled cards (PDF=red, Word=blue, Excel=green, PPT=orange, etc.) with file type labels. All file cards in both composer variants (default and document mode) are clickable via `onPreviewFile` prop, opening a dedicated preview modal (`previewUploadedFile` state in `chat-interface.tsx`). **PDF rendering uses `PdfPreview` component** (react-pdf v10 / pdfjs-dist v5.4.296 canvas-based rendering) — the pdf.js worker is loaded from the local `pdfjs-dist` package via `new URL('pdfjs-dist/build/pdf.worker.min.mjs', import.meta.url)` (CDN doesn't have v5.x). `DocumentPreview.tsx` also uses local `pdfjs-dist` import (not CDN) for version consistency. The PdfPreview provides page navigation, zoom, rotation, download, and fullscreen. **DOCX preview** uses server-side `mammoth` conversion via `GET /api/files/:id/preview-html` endpoint (returns `{type:"docx", html:"..."}`) rendered with `dangerouslySetInnerHTML` in prose styles. **XLSX preview** uses server-side `xlsx` package to extract sheet data as JSON arrays, rendered as HTML tables with sheet tabs. Text files (txt, md, json, csv, etc.) are returned as `{type:"text", content:"..."}` from the same endpoint. The standalone `FilePreviewModal.tsx` component handles all file types: PDF delegates to `PdfPreview`, images render inline, DOCX/XLSX/text fetch from `/api/files/:id/preview-html`, unsupported types show download fallback. Binary file access uses `/api/files/:id/raw` endpoint. CSP headers updated across all 3 security middleware files.
