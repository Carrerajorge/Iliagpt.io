...
The agent loop features production-grade reliability: exponential backoff retry for transient LLM API failures (429/503/502), a CircuitBreaker that trips after 3 consecutive tool failures (including logical errors), parallel tool execution when the model returns multiple tool_calls simultaneously, high-risk action detection with SSE `tool_requires_confirmation` events and trace logging, and automatic memory persistence of agent runs via RAG indexing. Token usage is tracked per run.

The RAG service (`server/services/ragService.ts`) provides RAGFlow-style document understanding with automatic chunking (800-char chunks with 200-char overlap), key phrase extraction, bigram/trigram local embeddings, and hybrid scoring (50% vector + 35% BM25 + 15% key phrase boost). File attachments in chat messages use Lucide icons (FileText, FileSpreadsheet, Presentation) with rich hover/shadow effects and click-to-preview functionality. The frontend ToolInvocationCard supports live streaming output, elapsed time counters, syntax-highlighted terminal output, clickable URLs, copy-to-clipboard, and rich output rendering for all tool types.
