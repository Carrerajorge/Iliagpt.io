...
**File Preview System** — Enhanced file preview in the composer bar uses `getFileTheme()` for consistent styled cards (PDF=red, Word=blue, Excel=green, PPT=orange, etc.) with file type labels. All file cards in both composer variants (default and document mode) are clickable via `onPreviewFile` prop, opening a dedicated preview modal (`previewUploadedFile` state in `chat-interface.tsx`). **PDF rendering uses `PdfPreview` component** (react-pdf v10 / pdf.js canvas-based rendering) which works in all browsers including Safari and embedded webviews — it renders PDFs on `<canvas>` elements, bypassing iframe/object/embed restrictions. The PdfPreview provides page navigation, zoom, rotation, download, and fullscreen. Images render inline, text/code via `<pre>`, and unsupported types show a download fallback. The chat message attachment preview modal (`previewFileAttachment` state, triggered by `handleOpenFileAttachmentPreview`) supports the same PDF rendering approach. The standalone `FilePreviewModal.tsx` component delegates PDF rendering to `PdfPreview` and handles images, text files, and download fallbacks for other types. Binary file access uses `/api/files/:id/raw` endpoint. CSP headers (`object-src`, `frame-src`, `worker-src`) updated across all 3 security middleware files to allow `blob:` URLs and `cdnjs.cloudflare.com` (for pdf.js worker).
