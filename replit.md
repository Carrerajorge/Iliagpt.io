...
**File Preview System** — Enhanced file preview in the composer bar uses `getFileTheme()` for consistent styled cards (PDF=red, Word=blue, Excel=green, PPT=orange, etc.) with file type labels. The chat message attachment preview modal (`chat-interface.tsx`) now supports PDF rendering (via iframe), image preview (inline `<img>`), text/markdown content, and fallback download for unsupported types. A standalone `FilePreviewModal.tsx` component is also available for reuse.
