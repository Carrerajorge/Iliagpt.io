...
Core agentic tools from OpenClaw v2026.3.1 are integrated directly, including `bash` (sandboxed shell execution), `web_fetch` (HTTP + HTML stripping), `web_search` (DuckDuckGo), `read_file`, `write_file`, `edit_file`, `list_files` (with path traversal protection), `rag_index_document` (deep document chunking with hybrid BM25+vector search), and `openclaw_rag_search` (semantic knowledge base search). These tools are dispatched via `executeOpenClawToolCall` and merged into the agent executor's tool array. The agent executor uses OpenAI-compatible API calls through OpenRouter, routing via `OPENAI_BASE_URL` with `minimax/minimax-m2.5` as the default model. The RAG service (`server/services/ragService.ts`) provides RAGFlow-style document understanding with automatic chunking (800-char chunks with 200-char overlap), key phrase extraction, bigram/trigram local embeddings, and hybrid scoring (50% vector + 35% BM25 + 15% key phrase boost). File attachments in chat messages use Lucide icons (FileText, FileSpreadsheet, Presentation) with rich hover/shadow effects and click-to-preview functionality.
