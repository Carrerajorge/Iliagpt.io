...
**File Preview System** — Enhanced file preview in the composer bar uses `getFileTheme()` for consistent styled cards (PDF=red, Word=blue, Excel=green, PPT=orange, etc.) with file type labels. All file cards in both composer variants (default and document mode) are clickable via `onPreviewFile` prop, opening a dedicated preview modal (`previewUploadedFile` state in `chat-interface.tsx`) that renders PDFs via iframe, images inline, text/code via `<pre>`, and a download fallback for unsupported types. The chat message attachment preview modal (`previewFileAttachment` state, triggered by `handleOpenFileAttachmentPreview`) supports PDF rendering (via iframe with `/api/files/:id/content`), image preview (inline `<img>`), text/markdown content, and fallback download. Images in the composer use `setPreviewUploadedImage` for a lightweight lightbox. A standalone `FilePreviewModal.tsx` component is also available for reuse.
